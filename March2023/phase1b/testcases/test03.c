...
int testcase_main()
{
    ...
    USLOSS_Console("EXPECTATION: main() creates many XXp1 children, and join()s all of them; then repeats twice.
      Process count is large enough to fill process table - meaning that students must free old process table entries.\n");
    ...
    for (j = 0; j < 3; j++) {
        ...
        for (i = 0; i < PROC_COUNT; i++) {
            ...
            USLOSS_Console("testcase_main(): after spork of child %d -- you will see this after the just-created child runs, and completes.
              So expect an alternation of XXp1() output, followed by these spork messages.\n", kidpid);
        }
