...
int testcase_main()
{
    ...
    for (j = 0; j < 2; j++) {
        for (i = 0; i < 5; i++) {
            ...
            USLOSS_Console("testcase_main(): calling spork(), arg_buf = '%s' -- testcase_main() will block until this child terminates, because the child is higher priority.\n", arg_buf);
            kidpid = spork("XXp1", XXp1, arg_buf, USLOSS_MIN_STACK, 2);
            USLOSS_Console("testcase_main(): after spork of child %d -- you should not see this until the child has ended.\n", kidpid);
        }
